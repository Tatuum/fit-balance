---
name: new-decision
description: Scaffold a new immutable engine-design decision file in docs/decisions/ for the fit-balance project — finds the next number, fills in the context/decision/consequences template, and updates the docs/decisions/README.md index. Use whenever a change to balance_points.py, scoring.py, or effects.yaml needs its rationale recorded, or when reversing/narrowing an earlier decision.
---

# New decision

Scaffolds one immutable file in `docs/decisions/` for an engine-level
design decision (a new formula, axis, scoring rule, or a reversal of a
prior one). See `docs/decisions/README.md` for why this project keeps
...
1. **Find the next number.** List `docs/decisions/*.md` (excluding
...
3. **Write `docs/decisions/NNNN-slug.md`**:
...
5. **Update `docs/decisions/README.md`**: add a row to the table in
...
6. **Update `NOTES.md`** to describe the resulting current-state behavior
   ...
   new decision file (`docs/decisions/NNNN-slug.md`). Follow the existing
